<script lang="ts">
	...
	const columnsVentasPorReferencia = ['fecha', 'sku', 'variante', 'cantidad', 'cantidad_%', 'valor', 'valor_%'];
    ...
    const columnsVentasPorMetadato = ['fecha', 'meta_valor', 'cantidad', 'cantidad_%', 'valor', 'valor_%']
    ...
    async function refreshVentasPorMetadato() {
        ...
        const metadatoIds = optionsMetaVaroles.filter((option) => option.selected).map((option) => parseInt(option.value));
...
{#if fVentas}
	<section class="flex w-full flex-col gap-5">
		...
		<form action="" class="flex items-center gap-5">
            ...
            <Select options={frecuenciaOptions()} label={'Frecuencia'} bind:value={frecuenciaPorReferenciaSelected}></Select>
		</form>
		<DataGrid data={fVentas} columns={columnsVentasPorReferencia} bind:rows={rowsDataGridReferencia} refresh_data={refreshVentasPorReferencia} />
	...
	<section class="flex w-full flex-col gap-5">
        ...
        <form action="" class="flex items-center gap-5">
            ...
            <Select options={frecuenciaOptions()} label={'Frecuencia'} bind:value={frecuenciaPorMetadatoSelected}></Select>
            <Select options={optionsAgrupacionMetadato} label="Agrupación" bind:value={agrupacionPorMetadatoSelected}></Select>
        </form>
        ...
        <DataGrid data={ventasPorMetadato} columns={columnsVentasPorMetadato} bind:rows={rowsDataGridMetadato} refresh_data={refreshVentasPorMetadato}></DataGrid>
	</section>
{/if}
